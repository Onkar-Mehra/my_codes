let filter_btnBrand = document.getElementsByClassName("filter_by_b_nam3");
let filter_btnDelivery = document.getElementsByClassName("filter_by_d");
let sort_btn = document.getElementsByClassName("sub_item_order");
...
function pop() {
    document.getElementById("hamburger_nav").style.height = "200px"
}
function close_nav() {
    document.getElementById("hamburger_nav").style.height = "0px"
}
...
function expand_filter() {
    ...
    if (filter_menu == "brand") {
        document.getElementById("filter_bb_name").style.display = 'flex'
    }
    else if (filter_menu == "Price") {
        document.getElementById("filter_bp").style.display = 'flex'
    }
    else if(filter_menu == 'Star Rating'){
        document.getElementById("filter_br").style.display = 'flex'
    }
    else {
        document.getElementById('filter_bd').style.display = 'flex'
    }
}
...
function expand_sort(){
    ...
    if((sort_menu == 'brand') || (sort_menu == 'price') || (sort_menu == 'rating')){
        document.getElementById("sub_item").style.display = 'flex'
    }
}

document.getElementById('quantity').innerText = filter_productByName.length

for (i=0; i<filter_btnBrand.length; i++){
    filter_btnBrand[i].addEventListener('click', (e) =>{
        e.preventDefault()
        document.getElementById("filter_bb_name").style.display = 'none'
...
function filter_price(a, b){
    document.getElementById("filter_bp").style.display = 'none'
...
function filter_rating(a){
    document.getElementById("filter_br").style.display = 'none'
...
for(i=0; i<filter_btnDelivery.length; i++){
    filter_btnDelivery[i].addEventListener('click', (e) =>{
        e.preventDefault()
        document.getElementById('filter_bd').style.display = 'none'
...
for(i=0; i<sort_btn.length; i++){
    sort_btn[i].addEventListener('click', (e) =>{
        document.getElementById("sub_item").style.display = 'none'
        let sort_menu = document.getElementById("sort").value;
...
function sort_by_brand(x, y){
    ...
    if(y == 'increasing'){
        ...
        for(i=0; i<div_brand_array.length; i++){
            ...
            let element = document.getElementById('catalogue');
